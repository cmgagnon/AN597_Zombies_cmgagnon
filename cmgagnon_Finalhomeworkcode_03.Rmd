...
#Couln't figure out how to get all the means in one command, but this gets the job done. I found the sapply function by googling "standard deviation in R". I believe it takes into account that not all columns are normal distributions.
```
...
#These are kind of basic but they work. Might try to add some color later.
```
...
#Nice looking basic scatterplots.method = "lm" gives the regression line.
```
...
```
All of the standard deviations are lower than the standard error calculated in question 5 except "zombies_killed".
...
shapiro_height <- NULL
for (i in 1:k){
    shapiro_height[i] <- shapiro.test(sz[[i]]$height)
}
shapiro.test(shapiro_height)

shapiro.test(sz$height)
shapiro.test(sz$weight)
shapiro.test(sz$age)
shapiro.test(sz$zombies_killed)
shapiro.test(sz$years_of_education)
```
I tried to graph the distributions but did not have much succes for soome reason.

#Challenges
1. Figuring out the different distributions.
2. Deciding with test to use to get 95% confident interval for non-normal distributions.
3. I wanted to try to add r-squared value to scatterplots but I got frustrated and gave up.
4.I ran into trouble doing the sub-sampling. I kept getting an error message saying I was sampling more then the number of samples in dataset. I figured it out but took a bit.
5.Some of the if/then stuff turned out to be more complicated then I expected.
6. last few part of question 6.
